...
def generate_trees(world):
    step_mas = [[0 for i in range(world_size)] for j in range(world_size)]
    for x in range(len(world)):
        for y in range(len(world)):
            if world[x][y] != 0 and world[x][y] != 5 and world[x][y] != 3:
                neib_tiles = [step_mas[x - 1][y], step_mas[x + 1][y], step_mas[x][y - 1], step_mas[x][y + 1], step_mas[x - 1][y - 1], step_mas[x + 1][y - 1], step_mas[x - 1][y + 1], step_mas[x + 1][y + 1]]
                if not 1 in neib_tiles:
...
#    world = resize_array(world, 2)
    ...
    world_objects = generate_trees(world)
...
world, world_objects = generate_world(seed)
